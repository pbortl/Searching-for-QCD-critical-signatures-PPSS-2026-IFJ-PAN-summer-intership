...
void Analysis::Run() {
    ...
    std::vector<double> cent_limits(2, 0.0); // Vector to hold the calculated centrality limits for 5%, 10% centrality
    std::vector<int> cent_events(2, 0); // Vector to hold the number of events corresponding to each centrality limit for plotting consistency
    double fracs[2] = {0.05, 0.10}; // Fractions for 5%, 10% centrality thresholds
    ...
    for (int i = 0; i < 2; ++i) {
        int bin = cumul->FindFirstBinAbove(fracs[i]); // Find the first bin in the cumulative histogram that exceeds the specified fraction (5%, 10%)
        if (bin > 0) {
    ...
    double dynamic_limit_10 = cent_limits[1]; 
    delete cumul;
    delete cumul_raw;

    std::cout << ">> Dynamic 10% centrality threshold calculated as: " << dynamic_limit_10 << " GeV\n" << std::endl;
        ...
        if (EventCuts::PassVertexZ(pevent->VertexZ)) { // Check if the event passes the Vertex Z cut    
            ...
            if (pevent->energyPSDPeripheralModules > Config::PSD_per_cut) { // Check if the event passes the PSD peripheral energy cut
                ...
                if (EventCuts::PassTracksRatio(pevent->nTracksAll, pevent->nTracksFit)) {
                    ...
                    if (pevent->energyPSDSelectedModules < dynamic_limit_10) { // Check if the event passes the dynamic centrality cut (10% most central events)
    ...
    std::cout << "Events surviving 0-10% cut: " << nEvents_after_centrality << std::endl;
